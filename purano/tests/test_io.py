...
from purano.io import read_tg_html_dir, read_tg_jsonl_dir, read_parsers_csv_dir
...
class TestTgHtmlReader(unittest.TestCase):
    def test_read_tg_html_dir(self):
        documents = []
        for document in read_tg_html_dir(TESTS_DATA_DIR):
...
class TestTgJsonlReader(unittest.TestCase):
    def test_read_tg_jsonl_dir(self):
        documents = []
        for document in read_tg_jsonl_dir(TESTS_DATA_DIR):
            assert_document(self, document)
            documents.append(document)
        self.assertEqual(len(documents), 2)
...
class TestCsvReader(unittest.TestCase):
    def test_read_parsers_csv_dir(self):
        documents = []
        for document in read_parsers_csv_dir(TESTS_DATA_DIR):
            assert_document(self, document)
            documents.append(document)
        self.assertEqual(len(documents), 2)
